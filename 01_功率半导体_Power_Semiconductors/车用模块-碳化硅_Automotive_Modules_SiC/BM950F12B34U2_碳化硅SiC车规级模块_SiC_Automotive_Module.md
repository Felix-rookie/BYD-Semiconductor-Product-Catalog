# 比亚迪半导体 (BYD) 车规级碳化硅 (SiC) 半桥模块 - BM950F12B34U2
...
**BM950F12B34U2** 是一款基于第三代宽禁带 **碳化硅 (SiC) MOSFET** 技术构建的高性能车规级功率模块。采用先进的 SiC MOSFET 工艺，具有极低的导通电阻、超低的开关损耗及卓越的高温工作能力。产品严格符合 **AEC-Q101** 标准，专为纯电/混动车主驱逆变器 (EV/HEV Traction Inverter) 设计，可显著提升系统效率与功率密度。
...
| 连续漏极电流 | I<sub>D</sub> | T<sub>C</sub> = 65 °C | 550 | A |
| 脉冲漏极电流 | I<sub>DM</sub> | 脉宽受限，符合 T<sub>jmax</sub> | 1800 | A |
| 总耗散功率 | P<sub>tot</sub> | T<sub>C</sub> = 25 °C | 1180 | W |
...
| :--- | :---: | :--- | :---: | :---: |
| 结-管壳热阻（每 SiC MOSFET） | R<sub>th(j-c)</sub> | - | 0.093 | °C/W |
| 管壳-散热片热阻 | R<sub>th(c-s)</sub> | 导热硅脂接触 | 参考规格书 | °C/W |
| 端子螺丝扭矩 | M | 功率端子安装螺钉 | 3.6 ~ 4.4 | N·m |
| 重量 | W | - | 690 | g |
...
| 漏-源极导通电阻 | R<sub>DS(on)</sub> | V<sub>GS</sub> = 18 V, I<sub>D</sub> = 420 A, T<sub>j</sub> = 25 °C<br>V<sub>GS</sub> = 18 V, T<sub>j</sub> = 150 °C | - | 1.9<br>4.6 | 3.2<br>- | mΩ |
...
| 门极总电荷 | Q<sub>g</sub> | V<sub>GS</sub> = [-5 ~ 18] V | - | 1.33 | - | μC |
| 内部门极电阻 | R<sub>Gint</sub> | - | - | 1.6 | - | Ω |
| 输入电容 | C<sub>iss</sub> | V<sub>DS</sub> = [电压] V, f = 1 MHz | - | 28 | - | nF |
| 输出电容 | C<sub>oss</sub> | V<sub>DS</sub> = [电压] V, f = 1 MHz | - | 1.61 | - | nF |
| 反向传输电容 | C<sub>rss</sub> | V<sub>DS</sub> = [电压] V, f = 1 MHz | - | 154 | - | nF |

### 2. SiC MOSFET 开关特性 (感性负载)

测试条件：V<sub>DD</sub> = 650 V, I<sub>D</sub> = 550 A, V<sub>GS</sub> = -5/+18, R<sub>G(on)</sub> = 12.4 Ω, R<sub>G(off)</sub> = 8.25 Ω

| 参数名称 (Parameter) | 符号 (Symbol) | 测试条件 (Conditions) | 典型值 | 单位 |
| :--- | :---: | :--- | :---: | :---: |
| 开通延迟时间 | t<sub>d(on)</sub> | T<sub>j</sub> = 25 °C<br>T<sub>j</sub> = 150 °C | 110<br>67 | ns |
| 上升时间 | t<sub>r</sub> | T<sub>j</sub> = 25 °C<br>T<sub>j</sub> = 150 °C | 113<br>99 | ns |
| 关断延迟时间 | t<sub>d(off)</sub> | T<sub>j</sub> = 25 °C<br>T<sub>j</sub> = 150 °C | 536<br>602 | ns |
| 下降时间 | t<sub>f</sub> | T<sub>j</sub> = 25 °C<br>T<sub>j</sub> = 150 °C | 71<br>75 | ns |
| 开通损耗 | E<sub>on</sub> | T<sub>j</sub> = 25 °C<br>T<sub>j</sub> = 150 °C | 30<br>27 | mJ |
| 关断损耗 | E<sub>off</sub> | T<sub>j</sub> = 25 °C<br>T<sub>j</sub> = 150 °C | 48<br>48 | mJ |
