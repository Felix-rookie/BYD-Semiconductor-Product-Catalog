# 比亚迪半导体 (BYD) 碳化硅 (SiC) 车规级功率模块 - BM700F12B34U4
...
**BM700F12B34U4** 是一款基于第三代半导体材料——碳化硅 (SiC) 的功率模块，采用先进的 SiC MOSFET 芯片，具有低损耗和高短路能力。采用银烧结工艺、直接冷却铜 PinFin 底板和高性能氮化硅陶瓷绝缘，最高支持 750V 电压平台。
...
| 连续漏极直流电流 | $I_{D(nom)}$ | $T_F = 65^\circ\text{C}, T_{vjmax} = 175^\circ\text{C}$ | 700 | A |
| 脉冲漏极电流 | $I_{D(pulse)}$ | 脉宽受限，符合 $T_{jmax}$ | 1400 | A |
| 总耗散功率 | $P_{tot}$ | $T_F = 65^\circ\text{C}$ | 910 | W |
...
| 参数名称 (Parameter) | 符号 (Symbol) | 测试条件 (Conditions) | 额定值 | 单位 |
| :--- | :---: | :--- | :---: | :---: |
| 体二极管连续正向电流 | $I_{SD}$ | 等同于 $I_{D(nom)}$ | 700 | A |
| 体二极管脉冲正向电流 | $I_{SD(pulse)}$ | 脉宽受限，符合 $T_{jmax}$ | 1400 | A |
...
| :--- | :---: | :--- | :---: | :---: | :---: | :---: |
| 漏极-源极导通电阻 | $R_{DS(on)}$ | $V_{GS}=18\text{V}, I_D=I_D=300A, V_GS=18V, T_j=25^\circ\text{C}$<br>$V_{GS}=18\text{V}, I_D=I_D=300A, V_GS=18V, T_j=175^\circ\text{C}$ | -<br>- | 2.75<br>6.6 (175°C) | 4.4<br>- | mΩ<br>mΩ |
...
| 门极充电电量-源极 | $Q_{gs}$ | $V_{DS}=850\text{V}, V_{GS}=-5~18\text{V}$ | - | 335 | - | nC |
| 门极充电电量-漏极 | $Q_{gd}$ | $V_{DS}=850\text{V}, V_{GS}=-5~18\text{V}$ | - | 180 | - | nC |
...
| 参数名称 (Parameter) | 符号 (Symbol) | 测试条件 (Conditions) | 最小值 | 典型值 | 最大值 | 单位 |
| :--- | :---: | :--- | :---: | :---: | :---: | :---: |
| 源极-漏极正向导通压降 | $V_{SD}$ | 按规格书 (Refer to Datasheet), $T_j=25^\circ\text{C}$<br>按规格书 (Refer to Datasheet), $T_j=175^\circ\text{C}$ | -<br>- | —<br>— | -<br>- | V<br>V |
