# 比亚迪半导体 (BYD) 车规级碳化硅 (SiC) 半桥模块 - BM1040F12B34U2
...
**BM1040F12B34U2** 是一款基于第三代宽禁带 **碳化硅 (SiC) MOSFET** 技术构建的高性能车规级功率模块。采用先进的 SiC MOSFET 工艺，具有极低的导通电阻、超低的开关损耗及卓越的高温工作能力。产品严格符合 **AEC-Q101** 标准，专为纯电/混动车主驱逆变器 (EV/HEV Traction Inverter) 设计，可显著提升系统效率与功率密度。
...
| 连续漏极电流 | I<sub>D</sub> | T<sub>C</sub> = 65 °C | 600 | A |
| 脉冲漏极电流 | I<sub>DM</sub> | 脉宽受限，符合 T<sub>jmax</sub> | 2000 | A |
| 总耗散功率 | P<sub>tot</sub> | T<sub>C</sub> = 25 °C | 1222 | W |
...
| :--- | :---: | :--- | :---: | :---: |
| 结-管壳热阻（每 SiC MOSFET） | R<sub>th(j-c)</sub> | - | 0.09 | °C/W |
...
| 漏-源极导通电阻 | R<sub>DS(on)</sub> | V<sub>GS</sub> = 18 V, I<sub>D</sub> = 480 A, T<sub>j</sub> = 25 °C<br>V<sub>GS</sub> = 18 V, T<sub>j</sub> = 150 °C | - | 1.7<br>4.0 | 2.8<br>- | mΩ |
| 栅极阈值电压 | V<sub>GS(th)</sub> | V<sub>DS</sub> = V<sub>GS</sub>, I<sub>D</sub> = [电流] mA | 2.1 | 3.2 | 5.8 | V |
| 栅极漏电流 | I<sub>GSS</sub> | V<sub>GS</sub> = ±20 V | - | - | 100 | nA |
| 漏极截止电流 | I<sub>DSS</sub> | V<sub>DS</sub> = V<sub>DSS</sub>, V<sub>GS</sub> = 0 V | - | - | 150 | μA |
| 体二极管正向电压 | V<sub>SD</sub> | V<sub>GS</sub> = 0 V, I<sub>F</sub> = [电流] A | - | 3.5 | 6.5 | V |
| 门极总电荷 | Q<sub>g</sub> | V<sub>GS</sub> = [-5 ~ 18] V | - | 1.52 | - | μC |
| 内部门极电阻 | R<sub>Gint</sub> | - | - | 1.4 | - | Ω |
| 输入电容 | C<sub>iss</sub> | V<sub>DS</sub> = [电压] V, f = 1 MHz | - | 32 | - | nF |
| 输出电容 | C<sub>oss</sub> | V<sub>DS</sub> = [电压] V, f = 1 MHz | - | 1.84 | - | nF |
| 反向传输电容 | C<sub>rss</sub> | V<sub>DS</sub> = [电压] V, f = 1 MHz | - | 176 | - | nF |

### 2. SiC MOSFET 开关特性 (感性负载)

测试条件：V<sub>DD</sub> = 650 V, I<sub>D</sub> = 600 A, V<sub>GS</sub> = -5/+18, R<sub>G(on)</sub> = 12.4 Ω, R<sub>G(off)</sub> = 8.25 Ω

| 参数名称 (Parameter) | 符号 (Symbol) | 测试条件 (Conditions) | 典型值 | 单位 |
| :--- | :---: | :--- | :---: | :---: |
| 开通延迟时间 | t<sub>d(on)</sub> | T<sub>j</sub> = 25 °C<br>T<sub>j</sub> = 150 °C | 159<br>122 | ns |
| 上升时间 | t<sub>r</sub> | T<sub>j</sub> = 25 °C<br>T<sub>j</sub> = 150 °C | 134<br>113 | ns |
| 关断延迟时间 | t<sub>d(off)</sub> | T<sub>j</sub> = 25 °C<br>T<sub>j</sub> = 150 °C | 540<br>625 | ns |
| 下降时间 | t<sub>f</sub> | T<sub>j</sub> = 25 °C<br>T<sub>j</sub> = 150 °C | 81<br>89 | ns |
| 开通损耗 | E<sub>on</sub> | T<sub>j</sub> = 25 °C<br>T<sub>j</sub> = 150 °C | 45<br>38 | mJ |
| 关断损耗 | E<sub>off</sub> | T<sub>j</sub> = 25 °C<br>T<sub>j</sub> = 150 °C | 51<br>51 | mJ |
