# 比亚迪半导体 (BYD) 碳化硅 (SiC) 车规级功率模块 - BM875B12J34U1
...
**BM875B12J34U1** 是一款基于第三代半导体材料——碳化硅 (SiC) 的功率模块，采用最新双面银烧结工艺，具有高导热、低损耗和高短路能力。内含先进的 SiC MOSFET 芯片，采用直接冷却铜 PinFin 底板和高性能氮化硅陶瓷绝缘。最高支持 900V 电压平台。

### 🌟 核心特性 (Features)
* 第三代半导体材料——碳化硅 (3rd Gen SiC)
* 双面银烧结工艺 (Double-sided Silver Sintering)
* 阻断电压 1200V (Blocking Voltage 1200V)
* 低 RDS(on) (Low RDS(on))
* 低开关损耗 (Low Switching Losses)
* 低 Qg 和 Crss (Low Qg and Crss)
* 低电感设计 Ls≤10nH (Low Inductive Design)
* 最高连续工作结温 175°C (Tvj op=175°C)
* 直接冷却铜 PinFin 底板
* 高性能氮化硅陶瓷绝缘
* 集成 NTC 温度传感器

### 🚗 典型应用 (Applications)
* 汽车级应用 (Automotive Applications)
* 混合动力车 (Hybrid Electrical Vehicles (H)EV)
* 电机驱动 (Motor Drives)
* 最高支持 900V 电压平台 (Max applied voltage platform: 900V)
...
| :--- | :---: | :---: | :---: | :---: | :---: | :---: |
| **BM875B12J34U1** | 1200 V | 875 A | 2.3 mΩ | 175 °C | 半桥模块 (Half-Bridge) | 标准模块封装 (PinFin Base Plate) |
...
| 连续漏极直流电流 | $I_{D(nom)}$ | $T_F = 65^\circ\text{C}, T_{vjmax} = 175^\circ\text{C}$ | 875 | A |
| 脉冲漏极电流 | $I_{D(pulse)}$ | 脉宽受限，符合 $T_{jmax}$ | 1060 | A |
| 总耗散功率 | $P_{tot}$ | $T_F = 65^\circ\text{C}$ | 1228 | W |
| 栅极-源极峰值电压 (瞬态) | $V_{GS}$ | $t_p < 1\mu\text{s}$ | $\pm$见规格书 | V |
| 栅极-源极电压 (最大值) | $V_{GSmax}$ | - | +22/-10 | V |
| 栅极-源极电压 (推荐工作) | $V_{GSop}$ | - | — | V |

### 2. 体二极管部分 (Body Diode)

| 参数名称 (Parameter) | 符号 (Symbol) | 测试条件 (Conditions) | 额定值 | 单位 |
| :--- | :---: | :--- | :---: | :---: |
| 体二极管连续正向电流 | $I_{SD}$ | 等同于 $I_{D(nom)}$ | 875 | A |
| 体二极管脉冲正向电流 | $I_{SD(pulse)}$ | 脉宽受限，符合 $T_{jmax}$ | 1060 | A |
...
| :--- | :---: | :--- | :---: | :---: | :---: | :---: |
| 漏极-源极导通电阻 | $R_{DS(on)}$ | $V_{GS}=18\text{V}, I_D=I_D=420A, V_GS=18V, T_j=25^\circ\text{C}$<br>$V_{GS}=18\text{V}, I_D=I_D=420A, V_GS=18V, T_j=175^\circ\text{C}$ | -<br>- | 2.3<br>4.3 (175°C) | 3.2<br>- | mΩ<br>mΩ |
| 漏极-源极击穿电压 | $V_{BR(DSS)}$ | $V_{GS}=0\text{V}, I_D=1\text{mA}$ | 1200 | - | - | V |
| 栅极-源极阈值电压 | $V_{GS(th)}$ | $I_D=$按规格书, $V_{DS}=V_{GS}, T_j=25^\circ\text{C}$ | 2.0 | — | 5.0 | V |
| 栅极漏电流 | $I_{GSS}$ | $V_{CE}=0\text{V}, V_{GS}=20\text{V}, T_j=25^\circ\text{C}$ | - | - | 500 nA | nA |
| 断态漏极漏电流 | $I_{DSS}$ | $V_{DS}=V_{DSS}, V_{GS}=0\text{V}, T_j=25^\circ\text{C}$ | - | - | 50 μA | $\mu\text{A}$ |
| 门极充电电量-源极 | $Q_{gs}$ | $V_{DS}=850\text{V}, V_{GS}=-5~18\text{V}$ | - | 287 | - | nC |
| 门极充电电量-漏极 | $Q_{gd}$ | $V_{DS}=850\text{V}, V_{GS}=-5~18\text{V}$ | - | 420 | - | nC |
| 门极总电量 | $Q_g$ | $V_{DS}=850\text{V}, V_{GS}=-5~18\text{V}$ | - | 1085 | - | nC |
| 内部栅极电阻 | $R_{Gint}$ | $T_j=25^\circ\text{C}, f=1\text{MHz}$ | - | 1.6 | - | $\Omega$ |
| 输入电容 | $C_{iss}$ | $f=1\text{MHz}, T_j=25^\circ\text{C}, V_{DS}=850\text{V}$ | - | 25.2 (nF) | - | nF |
| 输出电容 | $C_{oss}$ | $f=1\text{MHz}, T_j=25^\circ\text{C}, V_{DS}=850\text{V}$ | - | 1169 (pF) | - | nF |
| 反向传输电容 | $C_{rss}$ | $f=1\text{MHz}, T_j=25^\circ\text{C}, V_{DS}=850\text{V}$ | - | 210 (pF) | - | pF |
