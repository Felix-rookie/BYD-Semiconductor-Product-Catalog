# 比亚迪半导体 (BYD) 车规级 Half-Bridge IGBT 模块 - BG600B12ME4D-B
...
**BG600B12ME4D-B** 是一款采用先进硅基 Field Stop 技术构建的 半桥 车规级功率模块，内部集成了 IGBT 与续流二极管 (FWD) 的最优匹配。产品严格符合 **AEC-Q101** 汽车级可靠性标准，专为新能源汽车电机驱动系统 (EV Traction)、车载充电机 (OBC)、DCDC 变换器及 PTC 加热等高可靠性场景设计。
...
| :--- | :---: | :---: | :---: | :---: | :---: | :---: |
| **BG600B12ME4D-B** | 1200 V | 600 A | 2.16 V | 175 °C | 半桥 (Half-Bridge) | V-DUAL1 |
...
| 输入电容 | C<sub>ies</sub> | f = 1 MHz, V<sub>CE</sub> = 25 V, V<sub>GE</sub> = 0 V | - | 20 | - | nF |
| 反向传输电容 | C<sub>res</sub> | f = 1 MHz, V<sub>CE</sub> = 25 V, V<sub>GE</sub> = 0 V | - | 0.9 | - | nF |

### 2. IGBT 开关特性 (IGBT-Inverter, 感性负载)

测试条件：V<sub>CC</sub> = 600 V, I<sub>C</sub> = 600 A, V<sub>GE</sub> = ±15 V, R<sub>Gon</sub> = 1.0 Ω, R<sub>Goff</sub> = 6.7 Ω
...
| 开通损耗 | E<sub>on</sub> | T<sub>j</sub> = 25 °C<br>T<sub>j</sub> = 125 °C<br>T<sub>j</sub> = 150 °C | -<br>-<br>- | 40<br>55<br>60 | -<br>-<br>- | mJ |
| 关断损耗 | E<sub>off</sub> | T<sub>j</sub> = 25 °C<br>T<sub>j</sub> = 125 °C<br>T<sub>j</sub> = 150 °C | -<br>-<br>- | 44<br>60<br>68 | -<br>-<br>- | mJ |
...
| 反向恢复电荷 | Q<sub>rr</sub> | 测试条件同开关特性 | - | 85 | - | μC |
| 反向恢复损耗 | E<sub>rec</sub> | 测试条件同开关特性 | - | 25 | - | mJ |
...
> *注：BG600B12ME4D-B 采用先进硅基 Field Stop（场终止）技术，具有低导通压降和优化的开关特性。*
>
> *该产品严格符合 **AEC-Q101** 汽车级可靠性标准，专为新能源汽车电机驱动系统（EV Traction）、车载充电机（OBC）、DCDC 变换器及 PTC 加热等高可靠性场景设计。*
> ⚠️ **Rev.B 版本说明**：本型号为 BG600B12ME4D 的 Rev.B 改版，优化了内部布局和可靠性指标。