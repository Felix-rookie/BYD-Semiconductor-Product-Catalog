# 比亚迪半导体 (BYD) 工业级 IGBT半桥模块 - BG150B07X13L5
...
**BG150B07X13L5** 是一款采用高性能 IGBT 芯片、FRD 芯片和优化电气连接设计的硅基工业级 IGBT 半桥模块。该模块具有低导通损耗、低开关损耗和高短路耐量，适用于工业变频器、服伺系统和 UPS 等高可靠性功率变换场景。

### 🌟 核心特性 (Features)
* **半桥拓扑**：工业级半桥模块 (Half-bridge module)，额定 V<sub>CE</sub>=650 V、I<sub>C</sub>=150 A。
...
| :--- | :---: | :---: | :---: | :---: | :---: | :---: |
| **BG150B07X13L5** | 650 V | 150 A | 1.43 V | 175 °C | 半桥 (Half-Bridge) | 34mm Industrial |
...
| 短路安全工作区 | t<sub>psc</sub> | V<sub>GE</sub>≤15 V, V<sub>CC</sub>=325 V, V<sub>CEM</sub>≤650 V, T<sub>vj</sub>≤25 °C | None | μs |
...
| :--- | :---: | :--- | :---: | :---: | :---: | :---: |
| IGBT 结-外壳热阻 | R<sub>th(j-c)</sub> | 每个独立 IGBT (per IGBT) | — | 0.17 | — | K/W |
| 二极管 结-外壳热阻 | R<sub>th(j-c)</sub> | 每个独立二极管 (per diode) | — | 0.31 | — | K/W |
...
| 参数名称 (Parameter) | 符号 (Symbol) | 测试条件 (Conditions) | 最小值 | 典型值 | 最大值 | 单位 |
| :--- | :---: | :--- | :---: | :---: | :---: | :---: |
| 集电极-发射极饱和压降 | V<sub>CE(sat)</sub> | I<sub>C</sub>=150 A, V<sub>GE</sub>=15 V, T<sub>vj</sub>=25 °C<br>I<sub>C</sub>=150 A, V<sub>GE</sub>=15 V, T<sub>vj</sub>=150 °C | -<br>- | 1.43<br>1.67 | —<br>- | V |
| 栅极阈值电压 | V<sub>GE(th)</sub> | V<sub>CE</sub>=V<sub>GE</sub>, I<sub>C</sub>=3 mA, T<sub>vj</sub>=25 °C | 5.0 | 6.0 | 7.0 | V |
| 栅极电荷 | Q<sub>G</sub> | V<sub>GE</sub>=-15 V … +15 V | — | 2 | — | μC |
| 内部栅极电阻 | R<sub>gint</sub> | T<sub>vj</sub>=25 °C | — | 2 | — | Ω |
| 输入电容 | C<sub>ies</sub> | T<sub>vj</sub>=25 °C, f=1 MHz, V<sub>GE</sub>=0 V, V<sub>CE</sub>=25 V | — | 10.6 | — | nF |
| 反向传输电容 | C<sub>res</sub> | T<sub>vj</sub>=25 °C, f=1 MHz, V<sub>GE</sub>=0 V, V<sub>CE</sub>=25 V | — | 500 | — | nF |
...
| 参数名称 (Parameter) | 符号 (Symbol) | 测试条件 (Conditions) | 最小值 | 典型值 | 最大值 | 单位 |
| :--- | :---: | :--- | :---: | :---: | :---: | :---: |
| 开通延迟时间 | t<sub>d(on)</sub> | T<sub>vj</sub>=25 °C<br>T<sub>vj</sub>=150 °C | -<br>- | 90.3<br>93.5 | -<br>- | ns |
| 上升时间 | t<sub>r</sub> | T<sub>vj</sub>=25 °C<br>T<sub>vj</sub>=150 °C | -<br>- | 46.4<br>51.5 | -<br>- | ns |
| 关断延迟时间 | t<sub>d(off)</sub> | T<sub>vj</sub>=25 °C<br>T<sub>vj</sub>=150 °C | -<br>- | 173<br>200 | -<br>- | ns |
| 下降时间 | t<sub>f</sub> | T<sub>vj</sub>=25 °C<br>T<sub>vj</sub>=150 °C | -<br>- | 142<br>191 | -<br>- | ns |
| 开通损耗能量 | E<sub>on</sub> | T<sub>vj</sub>=25 °C<br>T<sub>vj</sub>=150 °C | -<br>- | 2.5<br>3.0 | -<br>- | mJ |
| 关断损耗能量 | E<sub>off</sub> | T<sub>vj</sub>=25 °C<br>T<sub>vj</sub>=150 °C | -<br>- | 6.0<br>7.4 | -<br>- | mJ |

### 3. 续流二极管特性 (Diode-Inverter)

| 参数名称 (Parameter) | 符号 (Symbol) | 测试条件 (Conditions) | 最小值 | 典型值 | 最大值 | 单位 |
| :--- | :---: | :--- | :---: | :---: | :---: | :---: |
| 正向电压 | V<sub>F</sub> | I<sub>F</sub>=150 A, V<sub>GE</sub>=0 V, T<sub>vj</sub>=25 °C<br>I<sub>F</sub>=150 A, V<sub>GE</sub>=0 V, T<sub>vj</sub>=150 °C | -<br>- | 1.46<br>1.44 | —<br>- | V |
| 恢复电荷 | Q<sub>rr</sub> | I<sub>F</sub>=150 A, V<sub>R</sub>=325 V, -di<sub>F</sub>/dt=— A/μs, T<sub>vj</sub>=125 °C | — | 5.6 | — | μC |
| 反向恢复损耗 | E<sub>rec</sub> | I<sub>F</sub>=150 A, V<sub>R</sub>=325 V, -di<sub>F</sub>/dt=— A/μs, T<sub>vj</sub>=125 °C | — | 1.32 | — | mJ |
