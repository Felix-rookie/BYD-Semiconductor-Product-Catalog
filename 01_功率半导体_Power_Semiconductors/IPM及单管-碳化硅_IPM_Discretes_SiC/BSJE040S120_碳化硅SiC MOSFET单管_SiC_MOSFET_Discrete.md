# 比亚迪半导体 (BYD) 碳化硅 (SiC) MOSFET 单管 - BSJE040S120
...
[![Package](https://img.shields.io/badge/Package-TO-263-7-orange.svg)]()
...
**BSJE040S120** 是一款专为汽车应用设计的碳化硅 (SiC) 功率 MOSFET，采用先进的工艺技术，实现极低的单位面积导通电阻。该器件适用于高频应用，具有出色的开关性能和可靠性。
...
| :--- | :---: | :---: | :---: | :---: | :---: | :---: |
| **BSJE040S120** | 1200 V | 60 A | 40 mΩ | 175 °C | 040S120 | TO-263-7 |
...
| 连续漏极电流 ($T_C = 25^\circ\text{C}$) | $I_D$ | 60 | A |
| 连续漏极电流 ($T_C = 100^\circ\text{C}$) | $I_D$ | 40 | A |
| 脉冲漏极电流 (脉宽受 $T_{jmax}$ 限制) | $I_{D(pulse)}$ | 100 | A |
| 栅极-源极电压 (最大值) | $V_{GSmax}$ | -8/+22 | V |
| 栅极-源极电压 (推荐工作) | $V_{GS}$ | -5/+18 | V |
| 总耗散功率 ($T_C = 25^\circ\text{C}$) | $P_D$ | 312 | W |
| 最高结温 | $T_{jmax}$ | 175 | °C |
...
| :--- | :---: | :---: | :---: |
| MOSFET 结-管壳热阻 | $R_{\theta JC}$ | 0.48 | °C/W |
| 结-环境热阻 | $R_{\theta JA}$ | 38 | °C/W |
...
| 漏极-源极导通电阻 | $R_{DS(on)}$ | V_GS=18V, I_D=33A, $T_j=25^\circ\text{C}$<br>V_GS=18V, I_D=33A, $T_j=175^\circ\text{C}$ | -<br>- | 40<br>68 | 60<br>- | mΩ<br>mΩ |
...
| 栅极-源极漏电流 | $I_{GSS}$ | $V_{GS}=18\text{V}, V_{DS}=0\text{V}$ | - | - | 200 | nA |
| 栅极电阻 | $R_G$ | $f=1\text{MHz}, V_{AC}=25\text{mV}$ | - | 2.5 | - | $\Omega$ |

### 2. 动态特性 (Dynamic Characteristic)

| 参数名称 (Parameter) | 符号 (Symbol) | 测试条件 (Conditions) | 典型值 (Typ.) | 单位 (Unit) |
| :--- | :---: | :--- | :---: | :---: |
| 输入电容 | $C_{iss}$ | $V_{DS}=800\text{V}, V_{GS}=0\text{V}, f=1\text{MHz}$ | 3009 | pF |
| 输出电容 | $C_{oss}$ | $V_{DS}=800\text{V}, V_{GS}=0\text{V}, f=1\text{MHz}$ | 182 | pF |
| 反向传输电容 | $C_{rss}$ | $V_{DS}=800\text{V}, V_{GS}=0\text{V}, f=1\text{MHz}$ | 25 | pF |
| 门极总电量 | $Q_G$ | $V_{DD}=800\text{V}, I_D$按规格书, $V_{GS}=-5/20\text{V}$ | 150 | nC |
| 栅-源电荷 | $Q_{gs}$ | $V_{DD}=800\text{V}, I_D$按规格书, $V_{GS}=-5/20\text{V}$ | 60 | nC |
| 栅-漏电荷 | $Q_{gd}$ | $V_{DD}=800\text{V}, I_D$按规格书, $V_{GS}=-5/20\text{V}$ | 30 | nC |
...
测试条件：$V_{{DS}}=800$V, $V_{{GS}}=-5/18$V, $I_D=40$A, $R_{{G(ext)}}=5\Omega$, $R_L=20\Omega$

| 参数分类 | 参数名称 (Parameter) | 符号 (Symbol) | 典型值 (Typ.) | 单位 (Unit) |
| :--- | :--- | :---: | :---: | :---: |
| **MOSFET 特性** | 开通延迟时间 (Turn-on delay time) | $t_{d(on)}$ | 22 | ns |
| | 上升时间 (Rise time) | $t_r$ | 46 | ns |
| | 关断延迟时间 (Turn-off delay time) | $t_{d(off)}$ | 40 | ns |
| | 下降时间 (Fall time) | $t_f$ | 52 | ns |
| | 开通损耗 (Turn-on energy) | $E_{on}$ | 1.3 | mJ |
| | 关断损耗 (Turn-off energy) | $E_{off}$ | 0.8 | mJ |
| **体二极管** | 源极-漏极正向电压 (Source-drain forward voltage) | $V_{SD}$ | 3.4 | V |
| | 反向恢复时间 (Reverse recovery time) | $t_{rr}$ | 50 | ns |
| | 恢复电荷 (Recovered charge) | $Q_{rr}$ | 140 | nC |
| | 峰值反向恢复电流 (Peak reverse recovery current) | $I_{rrm}$ | 5 | A |

---

## 📐 封装外形尺寸 (TO-263-7 Outline Dimensions)
