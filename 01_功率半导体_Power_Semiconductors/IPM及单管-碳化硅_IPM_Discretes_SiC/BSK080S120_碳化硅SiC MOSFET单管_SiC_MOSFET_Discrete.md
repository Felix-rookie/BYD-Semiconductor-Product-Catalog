# 比亚迪半导体 (BYD) 碳化硅 (SiC) MOSFET 单管 - BSK080S120
...
**BSK080S120** 是一款专为汽车应用设计的碳化硅 (SiC) 功率 MOSFET，采用先进的工艺技术，实现极低的单位面积导通电阻。该器件适用于高频应用，具有出色的开关性能和可靠性。
...
| :--- | :---: | :---: | :---: | :---: | :---: | :---: |
| **BSK080S120** | 1200 V | 56 A | 80 mΩ | 175 °C | BSK080S120 | TO-247-4 |
...
| 连续漏极电流 ($T_C = 25^\circ\text{C}$) | $I_D$ | 56 | A |
| 连续漏极电流 ($T_C = 100^\circ\text{C}$) | $I_D$ | 37 | A |
| 脉冲漏极电流 (脉宽受 $T_{jmax}$ 限制) | $I_{D(pulse)}$ | 96 | A |
| 栅极-源极电压 (最大值) | $V_{GSmax}$ | -10/+25 | V |
| 栅极-源极电压 (推荐工作) | $V_{GS}$ | -5/+20 | V |
| 总耗散功率 ($T_C = 25^\circ\text{C}$) | $P_D$ | 337 | W |
| 最高结温 | $T_{jmax}$ | 175 | °C |
...
| :--- | :---: | :---: | :---: |
| MOSFET 结-管壳热阻 | $R_{\theta JC}$ | 0.44 | °C/W |
...
| 漏极-源极导通电阻 | $R_{DS(on)}$ | V_GS=20V, I_D=28A, $T_j=25^\circ\text{C}$<br>V_GS=20V, I_D=28A, $T_j=175^\circ\text{C}$ | -<br>- | 80<br>119 | 120<br>- | mΩ<br>mΩ |
...
| :--- | :---: | :--- | :---: | :---: |
| 输入电容 | $C_{iss}$ | $V_{DS}=800\text{V}, V_{GS}=0\text{V}, f=1\text{MHz}$ | 2030 | pF |
| 输出电容 | $C_{oss}$ | $V_{DS}=800\text{V}, V_{GS}=0\text{V}, f=1\text{MHz}$ | 101 | pF |
| 反向传输电容 | $C_{rss}$ | $V_{DS}=800\text{V}, V_{GS}=0\text{V}, f=1\text{MHz}$ | 24 | pF |
| 门极总电量 | $Q_G$ | $V_{DD}=800\text{V}, I_D$按规格书, $V_{GS}=-5/20\text{V}$ | 88 | nC |
| 栅-源电荷 | $Q_{gs}$ | $V_{DD}=800\text{V}, I_D$按规格书, $V_{GS}=-5/20\text{V}$ | 27 | nC |
...
| | 关断延迟时间 (Turn-off delay time) | $t_{d(off)}$ | 39 | ns |
| | 下降时间 (Fall time) | $t_f$ | 17 | ns |
| | 开通损耗 (Turn-on energy) | $E_{on}$ | 1.7 | mJ |
| | 关断损耗 (Turn-off energy) | $E_{off}$ | 0.3 | mJ |
