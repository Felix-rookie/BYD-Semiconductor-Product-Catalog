# 比亚迪半导体 (BYD) 碳化硅 (SiC) MOSFET 单管 - BSN080S120
...
**BSN080S120** 是一款专为汽车应用设计的碳化硅 (SiC) 功率 MOSFET，采用先进的工艺技术，实现极低的单位面积导通电阻。该器件适用于高频应用，具有出色的开关性能和可靠性。
...
| :--- | :---: | :---: | :---: | :---: | :---: | :---: |
| **BSN080S120** | 1200 V | 36 A | 80 mΩ | 175 °C | BSN080S120 | TO-247 |
...
| 连续漏极电流 ($T_C = 25^\circ\text{C}$) | $I_D$ | 36 | A |
| 连续漏极电流 ($T_C = 100^\circ\text{C}$) | $I_D$ | 25 | A |
| 脉冲漏极电流 (脉宽受 $T_{jmax}$ 限制) | $I_{D(pulse)}$ | 60 | A |
| 栅极-源极电压 (最大值) | $V_{GSmax}$ | -15/+25 (AC>1Hz) | V |
| 栅极-源极电压 (推荐工作) | $V_{GS}$ | -5/+20 | V |
| 总耗散功率 ($T_C = 25^\circ\text{C}$) | $P_D$ | 238 | W |
| 最高结温 | $T_{jmax}$ | 175 | °C |
...
| :--- | :---: | :---: | :---: |
| MOSFET 结-管壳热阻 | $R_{\theta JC}$ | 0.63 | °C/W |
...
| 漏极-源极导通电阻 | $R_{DS(on)}$ | V_GS=20V, I_D=20A, $T_j=25^\circ\text{C}$<br>V_GS=20V, I_D=20A, $T_j=175^\circ\text{C}$ | -<br>- | 80<br>120 | 100<br>- | mΩ<br>mΩ |
| 栅极-源极阈值电压 | $V_{GS(th)}$ | $V_{DS}=V_{GS}, I_{DS}=10\text{mA}$ | 2.0 | — | 4.0 | V |
...
| 栅极电阻 | $R_G$ | $f=1\text{MHz}, V_{AC}=25\text{mV}$ | - | 5 | - | $\Omega$ |

### 2. 动态特性 (Dynamic Characteristic)

| 参数名称 (Parameter) | 符号 (Symbol) | 测试条件 (Conditions) | 典型值 (Typ.) | 单位 (Unit) |
| :--- | :---: | :--- | :---: | :---: |
| 输入电容 | $C_{iss}$ | $V_{DS}=800\text{V}, V_{GS}=0\text{V}, f=1\text{MHz}$ | 2080 | pF |
| 输出电容 | $C_{oss}$ | $V_{DS}=800\text{V}, V_{GS}=0\text{V}, f=1\text{MHz}$ | 97 | pF |
| 反向传输电容 | $C_{rss}$ | $V_{DS}=800\text{V}, V_{GS}=0\text{V}, f=1\text{MHz}$ | 22 | pF |
| 门极总电量 | $Q_G$ | $V_{DD}=800\text{V}, I_D$按规格书, $V_{GS}=-5/20\text{V}$ | 85 | nC |
| 栅-源电荷 | $Q_{gs}$ | $V_{DD}=800\text{V}, I_D$按规格书, $V_{GS}=-5/20\text{V}$ | 23 | nC |
| 栅-漏电荷 | $Q_{gd}$ | $V_{DD}=800\text{V}, I_D$按规格书, $V_{GS}=-5/20\text{V}$ | 26 | nC |
...
测试条件：$V_{{DS}}=800$V, $V_{{GS}}=-5/20$V, $I_D=20$A, $R_{{G(ext)}}=5\Omega$, $R_L=40\Omega$

| 参数分类 | 参数名称 (Parameter) | 符号 (Symbol) | 典型值 (Typ.) | 单位 (Unit) |
| :--- | :--- | :---: | :---: | :---: |
| **MOSFET 特性** | 开通延迟时间 (Turn-on delay time) | $t_{d(on)}$ | 23 | ns |
| | 上升时间 (Rise time) | $t_r$ | 60 | ns |
| | 关断延迟时间 (Turn-off delay time) | $t_{d(off)}$ | 17 | ns |
| | 下降时间 (Fall time) | $t_f$ | 12 | ns |
| | 开通损耗 (Turn-on energy) | $E_{on}$ | 180 (μJ) | μJ |
| | 关断损耗 (Turn-off energy) | $E_{off}$ | 70 (μJ) | μJ |
| **体二极管** | 源极-漏极正向电压 (Source-drain forward voltage) | $V_{SD}$ | 3.5 | V |
| | 反向恢复时间 (Reverse recovery time) | $t_{rr}$ | 38 | ns |
| | 恢复电荷 (Recovered charge) | $Q_{rr}$ | 207 | nC |
| | 峰值反向恢复电流 (Peak reverse recovery current) | $I_{rrm}$ | 13 | A |
