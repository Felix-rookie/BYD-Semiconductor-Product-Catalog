# 比亚迪半导体 (BYD) 车规级 碳化硅 (SiC) MOSFET - BSN160S120
...
**BSN160S120** 是一款基于第三代宽禁带 **碳化硅 (SiC) MOSFET** 技术构建的高性能功率开关器件。采用平面栅 SiC MOSFET 工艺，具有极低的导通电阻、超低的开关损耗及卓越的高温工作能力。本产品严格符合 **AEC-Q101** 车规级标准，专为高效率电动汽车驱动系统与高频工业电源设计。
...
| :--- | :---: | :---: | :---: | :---: | :---: |
| **BSN160S120** | 1200 V | 160 A | 160 mΩ | 175 °C | TO-247-4 |
...
| 连续漏极电流 (T<sub>C</sub> = 25 °C) | I<sub>D</sub> | 140 | A |
| 连续漏极电流 (T<sub>C</sub> = 100 °C) | I<sub>D</sub> | 90 | A |
| 脉冲漏极电流 | I<sub>DM</sub> | 300 | A |
| 栅-源极电压 | V<sub>GS</sub> | -8/+22 | V |
| 总耗散功率 (T<sub>C</sub> = 25 °C) | P<sub>tot</sub> | 680 | W |
...
| :--- | :---: | :---: | :---: |
| 结-管壳热阻 | R<sub>th(j-c)</sub> | 0.22 | °C/W |
...
| 漏-源极导通电阻 | R<sub>DS(on)</sub> | V<sub>GS</sub> = 18 V, I<sub>D</sub> = 120 A, T<sub>j</sub> = 25 °C<br>V<sub>GS</sub> = 18 V, I<sub>D</sub> = 120 A, T<sub>j</sub> = 150 °C | - | 160<br>272 | 240<br>- | mΩ |
| 栅极阈值电压 | V<sub>GS(th)</sub> | V<sub>DS</sub> = V<sub>GS</sub>, I<sub>D</sub> = 40 mA | 2.0 | 3.2 | 4.0 | V |
...
| 体二极管正向电压 | V<sub>SD</sub> | V<sub>GS</sub> = 0 V, I<sub>F</sub> = 120 A | - | 3.2 | - | V |

### 2. 动态特性 (Dynamic Characteristic)

| 参数名称 (Parameter) | 符号 (Symbol) | 测试条件 (Conditions) | 典型值 | 单位 |
| :--- | :---: | :--- | :---: | :---: |
| 输入电容 | C<sub>iss</sub> | V<sub>DS</sub> = 800 V, f = 1 MHz | 11000 | pF |
| 输出电容 | C<sub>oss</sub> | V<sub>DS</sub> = 800 V, f = 1 MHz | 700 | pF |
| 反向传输电容 | C<sub>rss</sub> | V<sub>DS</sub> = 800 V, f = 1 MHz | 90 | pF |
| 门极总电荷 | Q<sub>g</sub> | V<sub>GS</sub> = [-5/20] V | 600 | nC |
| 门极-漏极电荷 | Q<sub>gd</sub> | - | 120 | nC |
| 内部门极电阻 | R<sub>G(int)</sub> | f = 1 MHz | 1.5 | Ω |

### 3. 开关特性 (Switching Characteristic, 感性负载)

测试条件：V<sub>DD</sub> = 800 V, I<sub>D</sub> = 160 A, V<sub>GS</sub> = -5/18 V, R<sub>G</sub> = 1.5 Ω

| 参数名称 (Parameter) | 符号 (Symbol) | T<sub>j</sub> = 25 °C | T<sub>j</sub> = 150 °C | 单位 |
| :--- | :---: | :---: | :---: | :---: |
| 开通延迟时间 | t<sub>d(on)</sub> | 28 | 25 | ns |
| 上升时间 | t<sub>r</sub> | 62 | 58 | ns |
| 关断延迟时间 | t<sub>d(off)</sub> | 52 | 57 | ns |
| 下降时间 | t<sub>f</sub> | 68 | 72 | ns |
| 开通损耗 | E<sub>on</sub> | 4.8 | 5.2 | mJ |
| 关断损耗 | E<sub>off</sub> | 2.8 | 3.2 | mJ |

### 4. 体二极管特性 (Body Diode Characteristic)

| 参数名称 (Parameter) | 符号 (Symbol) | 测试条件 (Conditions) | 典型值 | 单位 |
| :--- | :---: | :--- | :---: | :---: |
| 反向恢复时间 | t<sub>rr</sub> | I<sub>F</sub> = 160 A, T<sub>j</sub> = 25 °C | 60 | ns |
| 反向恢复电荷 | Q<sub>rr</sub> | I<sub>F</sub> = 160 A, T<sub>j</sub> = 25 °C | 550 | nC |
| 反向恢复峰值电流 | I<sub>RM</sub> | - | 12 | A |
