# 比亚迪半导体 (BYD) 工业级 IGBT 单管 - BGF15T65SD-I
...
**BGF15T65SD-I** 是一款基于先进 TrenchFS 技术（Trench Field-Stop Technology）构建的高性能、高可靠性 IGBT 单管，内部集成了软快恢复反并联二极管。本产品为工业级标准，专为工业缝纫机及逆变器应用设计。
...
| :--- | :---: | :---: | :---: | :---: | :---: | :---: |
| **BGF15T65SD-I** | 650 V | 15 A | 1.6 V | 175 °C | 15T65SD-I | TO220F-3 |
...
| 焊接温度 (距管壳 1.6mm 处限时 10s) | T<sub>st</sub> | 300 | °C |
...
| 二极管结-管壳热阻 | R<sub>th(j-c)</sub> | 8.0 | °C/W |
...
| 集电极-发射极饱和电压 | V<sub>CE(sat)</sub> | V<sub>GE</sub>=15 V, I<sub>C</sub>=15 A, T<sub>j</sub>=25 °C<br>V<sub>GE</sub>=15 V, I<sub>C</sub>=15 A, T<sub>j</sub>=150 °C | - | 1.6<br>1.8 | 1.8<br>- | V |
...
| 短路电流 | I<sub>C(sc)</sub> | V<sub>CC</sub>=400 V, V<sub>GE</sub>=15 V, T<sub>j</sub>=150 °C | 60 | A |
| 反偏安全工作区 | RBSOA | - | - | A |
...
| | 下降时间 (Fall time) | t<sub>f</sub> | 115 | ns |
| | 开通损耗 (Turn-on energy)* | E<sub>on</sub> | 0.21 | mJ |
| | 关断损耗 (Turn-off energy) | E<sub>off</sub> | 0.26 | mJ |
| | 总开关损耗 (Total switching energy) | E<sub>ts</sub> | 0.47 | mJ |
...
| 参数分类 | 参数名称 (Parameter) | 符号 (Symbol) | 典型值 (Typ.) | 单位 (Unit) |
| :--- | :--- | :---: | :---: | :---: |
| **IGBT 特性** | 开通延迟时间 (Turn-on delay time) | t<sub>d(on)</sub> | 40 | ns |
| | 上升时间 (Rise time) | t<sub>r</sub> | 40 | ns |
| | 关断延迟时间 (Turn-off delay time) | t<sub>d(off)</sub> | 130 | ns |
| | 下降时间 (Fall time) | t<sub>f</sub> | 145 | ns |
| | 开通损耗 (Turn-on energy)* | E<sub>on</sub> | 0.28 | mJ |
| | 关断损耗 (Turn-off energy) | E<sub>off</sub> | 0.54 | mJ |
| | 总开关损耗 (Total switching energy) | E<sub>ts</sub> | 0.82 | mJ |
