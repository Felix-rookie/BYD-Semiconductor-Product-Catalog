# 比亚迪半导体 (BYD) 工业级 IGBT 单管 - BGN30T65HD
...
**BGN30T65HD** 是一款基于先进 TrenchFS 技术（Trench Field-Stop Technology）构建的高性能、高可靠性 IGBT 单管，内部集成了软快恢复反并联二极管。本产品为工业级标准，专为 PFC 功率因数校正应用设计。

### 🌟 核心特性 (Features)
* **尖端技术**：650V Trench 场终止技术 (Trench Field-Stop Technology)
* **超快开关**：极低的开关损耗，超快关断特性 (Ultra-fast switching)
...
| :--- | :---: | :---: | :---: | :---: | :---: | :---: |
| **BGN30T65HD** | 650 V | 30 A | 1.85 V | 175 °C | 30T65HD | TO247-3 |
...
| 集电极-发射极饱和电压 | V<sub>CE(sat)</sub> | V<sub>GE</sub>=15 V, I<sub>C</sub>=30 A, T<sub>j</sub>=25 °C<br>V<sub>GE</sub>=15 V, I<sub>C</sub>=30 A, T<sub>j</sub>=150 °C | - | 1.85<br>2.2 | 2.3<br>- | V |
| 二极管正向导通电压 | V<sub>F</sub> | V<sub>GE</sub>=0 V, I<sub>F</sub>=15 A, T<sub>j</sub>=25 °C<br>V<sub>GE</sub>=0 V, I<sub>F</sub>=15 A, T<sub>j</sub>=150 °C | - | 1.7<br>1.4 | 2.5<br>- | V |
...
| | 上升时间 (Rise time) | t<sub>r</sub> | 66 | ns |
| | 关断延迟时间 (Turn-off delay time) | t<sub>d(off)</sub> | 65 | ns |
| | 下降时间 (Fall time) | t<sub>f</sub> | 16 | ns |
| | 开通损耗 (Turn-on energy)* | E<sub>on</sub> | 1.12 | mJ |
| | 关断损耗 (Turn-off energy) | E<sub>off</sub> | 0.34 | mJ |
| | 总开关损耗 (Total switching energy) | E<sub>ts</sub> | 1.46 | mJ |
...
| | 关断延迟时间 (Turn-off delay time) | t<sub>d(off)</sub> | 76 | ns |
| | 下降时间 (Fall time) | t<sub>f</sub> | 54 | ns |
| | 开通损耗 (Turn-on energy)* | E<sub>on</sub> | 1.42 | mJ |
| | 关断损耗 (Turn-off energy) | E<sub>off</sub> | 0.59 | mJ |
| | 总开关损耗 (Total switching energy) | E<sub>ts</sub> | 2.01 | mJ |
