# 比亚迪半导体 (BYD) 工业级 IGBT 单管 - BGN20T65SD
...
**BGN20T65SD** 是一款基于先进 TrenchFS 技术（Trench Field-Stop Technology）构建的高性能、高可靠性 IGBT 单管，内部集成了软快恢复反并联二极管。本产品为工业级标准，专为通用变频器应用设计。
...
| :--- | :---: | :---: | :---: | :---: | :---: | :---: |
| **BGN20T65SD** | 650 V | 20 A | 1.6 V | 175 °C | 20T65SD | TO247-3 |
...
| 连续集电极电流 (T<sub>C</sub> = 25 °C) | I<sub>C</sub> | 40 | A |
| 连续集电极电流 (T<sub>C</sub> = 100 °C) | I<sub>C</sub> | 20 | A |
| 集电极脉冲电流 (脉宽受 T<sub>jmax</sub> 限制) | I<sub>Cpuls</sub> | 80 | A |
| 二极管正向直流电流 (T<sub>C</sub> = 25 °C) | I<sub>F</sub> | 40 | A |
| 二极管正向直流电流 (T<sub>C</sub> = 100 °C) | I<sub>F</sub> | 20 | A |
| 二极管脉冲电流 | I<sub>Fpuls</sub> | 80 | A |
| 栅极-发射极电压 | V<sub>GE</sub> | ± 20 | V |
| 短路耐受时间 (V<sub>GE</sub>=15 V, V<sub>CC</sub>≤400 V, T<sub>j</sub>≤150 °C) | t<sub>sc</sub> | 5 | μs |
| 总耗散功率 (T<sub>C</sub> = 25 °C) | P<sub>tot</sub> | 192 | W |
...
| :--- | :---: | :---: | :---: |
| IGBT 结-管壳热阻 | R<sub>th(j-c)</sub> | 0.78 | °C/W |
| 二极管结-管壳热阻 | R<sub>th(j-c)</sub> | 2.07 | °C/W |
...
| 集电极-发射极饱和电压 | V<sub>CE(sat)</sub> | V<sub>GE</sub>=15 V, I<sub>C</sub>=20 A, T<sub>j</sub>=25 °C<br>V<sub>GE</sub>=15 V, I<sub>C</sub>=20 A, T<sub>j</sub>=150 °C | - | 1.6<br>1.8 | 2.0<br>- | V |
| 二极管正向导通电压 | V<sub>F</sub> | V<sub>GE</sub>=0 V, I<sub>F</sub>=20 A, T<sub>j</sub>=25 °C<br>V<sub>GE</sub>=0 V, I<sub>F</sub>=20 A, T<sub>j</sub>=150 °C | - | 1.7<br>1.4 | 2.4<br>- | V |
...
| :--- | :---: | :--- | :---: | :---: |
| 输入电容 | C<sub>ies</sub> | V<sub>CE</sub>=25 V, V<sub>GE</sub>=0 V, f=1 MHz | 1500 | pF |
| 输出电容 | C<sub>oes</sub> | V<sub>CE</sub>=25 V, V<sub>GE</sub>=0 V, f=1 MHz | 128 | pF |
| 反向传输电容 | C<sub>res</sub> | V<sub>CE</sub>=25 V, V<sub>GE</sub>=0 V, f=1 MHz | 28.7 | pF |
| 门极总电量 | Q<sub>G</sub> | V<sub>CC</sub>=400 V, I<sub>C</sub>=30 A, V<sub>GE</sub>=15 V | 43.9 | nC |
| 内部发射极电感 | L<sub>E</sub> | - | - | nH |
| 短路电流 | I<sub>C(sc)</sub> | V<sub>CC</sub>=400 V, V<sub>GE</sub>=15 V, T<sub>j</sub>=150 °C | 80 | A |
| 反偏安全工作区 | RBSOA | - | - | A |
...
测试条件：V<sub>CC</sub>=400 V, I<sub>C</sub>=20 A, V<sub>GE</sub>=-7.5/15 V, R<sub>G</sub>=10 Ω

| 参数分类 | 参数名称 (Parameter) | 符号 (Symbol) | 典型值 (Typ.) | 单位 (Unit) |
| :--- | :--- | :---: | :---: | :---: |
| **IGBT 特性** | 开通延迟时间 (Turn-on delay time) | t<sub>d(on)</sub> | 40.5 | ns |
| | 上升时间 (Rise time) | t<sub>r</sub> | 48 | ns |
| | 关断延迟时间 (Turn-off delay time) | t<sub>d(off)</sub> | 70 | ns |
| | 下降时间 (Fall time) | t<sub>f</sub> | 141 | ns |
| | 开通损耗 (Turn-on energy)* | E<sub>on</sub> | 0.55 | mJ |
| | 关断损耗 (Turn-off energy) | E<sub>off</sub> | 0.26 | mJ |
| | 总开关损耗 (Total switching energy) | E<sub>ts</sub> | 0.81 | mJ |
| **反并联二极管** | 反向恢复时间 (Reverse recovery time) | t<sub>rr</sub> | 117 | ns |
| | 恢复电荷 (Recovered charge) | Q<sub>rr</sub> | 345 | nC |
| | 反向恢复峰值电流 (Peak reverse recovery current) | I<sub>RM</sub> | 5.3 | A |
...
| 参数分类 | 参数名称 (Parameter) | 符号 (Symbol) | 典型值 (Typ.) | 单位 (Unit) |
| :--- | :--- | :---: | :---: | :---: |
| **IGBT 特性** | 开通延迟时间 (Turn-on delay time) | t<sub>d(on)</sub> | 39.5 | ns |
| | 上升时间 (Rise time) | t<sub>r</sub> | 47 | ns |
| | 关断延迟时间 (Turn-off delay time) | t<sub>d(off)</sub> | 80 | ns |
| | 下降时间 (Fall time) | t<sub>f</sub> | 166 | ns |
| | 开通损耗 (Turn-on energy)* | E<sub>on</sub> | 0.71 | mJ |
| | 关断损耗 (Turn-off energy) | E<sub>off</sub> | 0.42 | mJ |
| | 总开关损耗 (Total switching energy) | E<sub>ts</sub> | 1.13 | mJ |
| **反并联二极管** | 反向恢复时间 (Reverse recovery time) | t<sub>rr</sub> | 152 | ns |
| | 恢复电荷 (Recovered charge) | Q<sub>rr</sub> | 980 | nC |
| | 反向恢复峰值电流 (Peak reverse recovery current) | I<sub>RM</sub> | 8.7 | A |
| | 反向恢复损耗 (Reverse recovered energy) | E<sub>rec</sub> | - | mJ |
