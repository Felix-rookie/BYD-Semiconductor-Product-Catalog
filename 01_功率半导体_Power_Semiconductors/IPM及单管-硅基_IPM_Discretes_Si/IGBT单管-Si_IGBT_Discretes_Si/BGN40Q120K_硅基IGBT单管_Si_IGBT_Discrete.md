# 比亚迪半导体 (BYD) 车规级 IGBT 单管 - BGN40Q120K
...
**BGN40Q120K** 是一款基于先进平面栅场终止技术（Planar Field-Stop Technology）构建的高性能、高可靠性 IGBT 单管，内部不集成反并联二极管（IGBT 单管拓扑）。本产品严格符合 **AEC-Q101** 车规级标准要求，专为汽车 PTC 加热及短路保护应用设计。

### 🌟 核心特性 (Features)
* **尖端技术**：1200V 平面栅场终止技术 (Planar Field-Stop Technology)
* **低饱和压降**：极低的饱和导通电压 (Low saturation voltage)
* **温漂特性**：饱和导通电压 (V<sub>CE(sat)</sub>) 具有正温度系数，利于并联拓展
* **短路强韧**：具备高达 10 μs 的短路耐受承受能力
* **单管拓扑**：IGBT 单管，无内置反并联二极管（适合外部独立续流设计）

### 🚗 典型应用 (Applications)
* 汽车 PTC 加热方案 (Electric Automotive PTC Heater)
* 短路保护器 (Short-Circuit Protector)
...
| :--- | :---: | :---: | :---: | :---: | :---: | :---: |
| **BGN40Q120K** | 1200 V | 40 A | 1.6 V | 175 °C | 40Q120K | TO247-3 |
...
| 二极管结-管壳热阻 | R<sub>th(j-c)</sub> | - | °C/W |
...
| 集电极-发射极饱和电压 | V<sub>CE(sat)</sub> | V<sub>GE</sub>=15 V, I<sub>C</sub>=40 A, T<sub>j</sub>=25 °C<br>V<sub>GE</sub>=15 V, I<sub>C</sub>=40 A, T<sub>j</sub>=150 °C | - | 1.6<br>1.8 | 2.0<br>- | V |
...
| 反偏安全工作区 | RBSOA | V<sub>CC</sub>=800 V, V<sub>GE</sub>=20 V, T<sub>j</sub>≤150 °C | 80 | A |
...
| :--- | :--- | :---: | :---: | :---: |
| **IGBT 特性** | 开通延迟时间 (Turn-on delay time) | t<sub>d(on)</sub> | 45 | ns |
| | 上升时间 (Rise time) | t<sub>r</sub> | 90 | ns |
| | 关断延迟时间 (Turn-off delay time) | t<sub>d(off)</sub> | 240 | ns |
| | 下降时间 (Fall time) | t<sub>f</sub> | 575 | ns |
| | 开通损耗 (Turn-on energy) | E<sub>on</sub> | 3.2 | mJ |
| | 关断损耗 (Turn-off energy) | E<sub>off</sub> | 7.9 | mJ |
| | 总开关损耗 (Total switching energy) | E<sub>ts</sub> | 11.1 | mJ |

*\*注：本器件为 IGBT 单管，无内置反并联二极管，不含二极管恢复特性。*
...
| | 上升时间 (Rise time) | t<sub>r</sub> | 96 | ns |
| | 关断延迟时间 (Turn-off delay time) | t<sub>d(off)</sub> | 292 | ns |
| | 下降时间 (Fall time) | t<sub>f</sub> | 812 | ns |
| | 开通损耗 (Turn-on energy) | E<sub>on</sub> | 4.8 | mJ |
| | 关断损耗 (Turn-off energy) | E<sub>off</sub> | 9.95 | mJ |
| | 总开关损耗 (Total switching energy) | E<sub>ts</sub> | 14.75 | mJ |
