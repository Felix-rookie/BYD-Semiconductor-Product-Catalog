# 比亚迪半导体 (BYD) 家电级 IPM 智能功率模块 - BIPE60015G-TE
...
**BIPE60015G-TE** 是一款集成度极高的超小型封装三相逆变智能功率模块 (IPM)。该模块采用先进设计，在极度紧凑的封装内实现了优化匹配的门极驱动与低损耗 IGBT 功率器件，并内置了全面的电路保护功能。由于采用了陶瓷覆铜板 (DBC) 和分立的负直流端子设计，产品在超小体积下实现了优异的低热阻性能与独立的相电流监测，同时内置温度模拟电压输出功能。
...
| :--- | :---: | :---: | :---: | :---: | :---: |
| **BIPE60015G-TE** | 600 V | 15 A | 1500 Vrms/min | 三相六管全桥 (Three-Phase Inverter) | BIP25-3824 |
...
| :--- | :--- | :--- | :---: | :---: |
| R<sub>th(j-c)Q</sub> | 结点-壳体热阻 (Junction to case) | 逆变器 IGBT 部分 (每 1/6 模块) | 3.0 | °C/W |
...
| :--- | :--- | :--- | :---: | :---: | :---: | :---: |
| V<sub>CE(SAT)</sub> | 集电极-发射极饱和电压 | V<sub>D</sub> = V<sub>BS</sub> = 15 V, V<sub>IN</sub> = 5 V, I<sub>C</sub> = 15 A, T<sub>J</sub> = 25 °C | - | 1.5 | 2.0 | V |
...
| :--- | :--- | :--- | :---: | :---: | :---: | :---: |
| **高端上桥臂**<br>(High-side) | t<sub>ON</sub> | 开通时间 (Turn-on time) | - | 750 | - | ns |
| | t<sub>c(ON)</sub> | 开通交叉时间 (Turn-on across time) | - | 75 | - | ns |
| | t<sub>rr</sub> | 反向恢复时间 (Reverse recovery time) | - | 100 | - | ns |
| | t<sub>OFF</sub> | 关断时间 (Turn-off time) | - | 1100 | - | ns |
| | t<sub>c(OFF)</sub> | 关断交叉时间 (Turn-off across time) | - | 150 | - | ns |
| **低端下桥臂**<br>(Low-side) | t<sub>ON</sub> | 开通时间 (Turn-on time) | - | 890 | - | ns |
| | t<sub>c(ON)</sub> | 开通交叉时间 (Turn-on across time) | - | 73 | - | ns |
| | t<sub>rr</sub> | 反向恢复时间 (Reverse recovery time) | - | 100 | - | ns |
| | t<sub>OFF</sub> | 关断时间 (Turn-off time) | - | 1100 | - | ns |
...
| I<sub>QBS</sub> | 每相 VBS 静态电流 | V<sub>BS</sub> = 15 V, IN<sub>(HU, HV, HW)</sub> = 0 V，自举回路 | - | 80 | 150 | μA |
...
| V<sub>CIN(ref)</sub> | 短路保护触发电压阈值 | T<sub>C</sub> = -40 ~ 100 °C, V<sub>D</sub> = 15 V（注 3）| 0.455 | 0.48 | 0.505 | V |
