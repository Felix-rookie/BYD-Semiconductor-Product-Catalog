# 比亚迪半导体 (BYD) 家电级 IPM 智能功率模块 - BIPN60030C
...
**BIPN60030C** 是一款高度集成的先进三相逆变智能功率模块 (IPM)。该模块结合了优化匹配的门极驱动与低损耗 IGBT 功率器件，并内置了全面的电路保护功能。由于采用了陶瓷覆铜板 (DBC) 和分立的负直流端子设计，产品在极度紧凑的体积下实现了优异的低热阻性能与独立的相电流监测。
...
| :--- | :---: | :---: | :---: | :---: | :---: |
| **BIPN60030C** | 600 V | 30 A | 2500 Vrms/min | 三相六管全桥 (Three-Phase Inverter) | DIP27-4426 |
...
| P<sub>C</sub> | 集电极功耗 (Collector dissipation) | T<sub>C</sub> = 25 °C, 每一片小芯片 | 128 | W |
...
| :--- | :--- | :--- | :---: | :---: |
| R<sub>th(j-c)Q</sub> | 结点-壳体热阻 (Junction to case) | 逆变器 IGBT 部分 (每 1/6 模块) | 0.78 | °C/W |
| R<sub>th(j-c) F</sub> | 结点-壳体热阻 (Junction to case) | 逆变器 FRD 续流二极管部分 (每 1/6 模块) | 1.43 | °C/W |
...
| V<sub>F</sub> | FRD 正向导通电压 | V<sub>IN</sub> = 0 V, I<sub>C</sub> = 20 A, T<sub>J</sub> = 25 °C | - | 1.7 | 2.2 | V |
| I<sub>CES</sub> | 集电极-发射极间漏电流 | V<sub>CE</sub> = V<sub>CES</sub>, V<sub>GE</sub> = 0 V, T<sub>J</sub> = 25 °C | - | - | 0.1 | mA |

### 2. 开关动态特性 (Switching Times, 感性负载测试)
测试条件：V<sub>PN</sub> = 300 V, V<sub>D</sub> = V<sub>BS</sub> = 15 V, I<sub>C</sub> = 30 A, V<sub>IN</sub> = 0 ↔ 5 V, T<sub>J</sub> = 25 °C（注 2）

| 驱动区域 | 符号 (Symbol) | 参数说明 (Parameter) | 最小值 | 典型值 | 最大值 | 单位 |
| :--- | :--- | :--- | :---: | :---: | :---: | :---: |
| **高端上桥臂**<br>(High-side) | t<sub>ON</sub> | 开通时间 (Turn-on time) | - | 700 | - | ns |
| | t<sub>c(ON)</sub> | 开通交叉时间 (Turn-on across time) | - | 140 | - | ns |
| | t<sub>OFF</sub> | 关断时间 (Turn-off time) | - | 1200 | - | ns |
| | t<sub>c(OFF)</sub> | 关断交叉时间 (Turn-off across time) | - | 130 | - | ns |
| **低端下桥臂**<br>(Low-side) | t<sub>ON</sub> | 开通时间 (Turn-on time) | - | 870 | - | ns |
| | t<sub>c(ON)</sub> | 开通交叉时间 (Turn-on across time) | - | 250 | - | ns |
| | t<sub>OFF</sub> | 关断时间 (Turn-off time) | - | 1200 | - | ns |
| | t<sub>c(OFF)</sub> | 关断交叉时间 (Turn-off across time) | - | 250 | - | ns |
...
| t<sub>FO</sub> | 故障输出脉冲宽度 | 采样端滤波配置电容 C<sub>FO</sub> = 26 nF（注 4）<br>采样端滤波配置电容 C<sub>FO</sub> = 33 nF（注 4） | -<br>- | 1.80<br>2.30 | -<br>- | ms |
