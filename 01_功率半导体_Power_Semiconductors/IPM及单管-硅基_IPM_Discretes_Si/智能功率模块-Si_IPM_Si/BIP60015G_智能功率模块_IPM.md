# 比亚迪半导体 (BYD) 家电级 IPM 智能功率模块 - BIP60015G
...
**BIP60015G** 是一款集成度极高的超小型封装三相逆变智能功率模块 (IPM)。该模块采用先进设计，在极度紧凑的封装内实现了优化匹配的门极驱动与低损耗 IGBT 功率器件，并内置了全面的电路保护功能。由于采用了陶瓷覆铜板 (DBC) 和分立的负直流端子设计，产品在超小体积下实现了优异的低热阻性能与独立的相电流监测，同时内置温度模拟电压输出功能。
...
| :--- | :---: | :---: | :---: | :---: | :---: |
| **BIP60015G** | 600 V | 15 A | 1500 Vrms/min | 三相六管全桥 (Three-Phase Inverter) | BIP25-3824 |
...
| P<sub>C</sub> | 集电极功耗 (Collector dissipation) | T<sub>C</sub> = 25 °C, 每一片小芯片 | 41 | W |
...
| T<sub>C</sub> | 模块壳体工作温度 (Module case operation temperature) | -40 °C ≤ T<sub>J</sub> ≤ 150 °C, 壳温检测位置详见尺寸图 | -40 ~ +125 | °C |
...
| :--- | :--- | :--- | :---: | :---: |
| R<sub>th(j-c)Q</sub> | 结点-壳体热阻 (Junction to case) | 逆变器 IGBT 部分 (每 1/6 模块) | 2.85 | °C/W |
| R<sub>th(j-c)F</sub> | 结点-壳体热阻 (Junction to case) | 逆变器 FRD 续流二极管部分 (每 1/6 模块) | 3.8 | °C/W |
...
| :--- | :--- | :--- | :---: | :---: | :---: | :---: |
| V<sub>CE(SAT)</sub> | 集电极-发射极饱和电压 | V<sub>D</sub> = V<sub>BS</sub> = 15 V, V<sub>IN</sub> = 5 V, I<sub>C</sub> = 15 A, T<sub>J</sub> = 25 °C | - | 1.7 | 2.2 | V |
| V<sub>F</sub> | FRD 正向导通电压 | V<sub>IN</sub> = 0 V, I<sub>C</sub> = 15 A, T<sub>J</sub> = 25 °C | - | 1.8 | 2.3 | V |
...
测试条件：V<sub>PN</sub> = 400 V, V<sub>D</sub> = V<sub>BS</sub> = 15 V, I<sub>C</sub> = 15 A, V<sub>IN</sub> = 0 ↔ 5 V, T<sub>J</sub> = 25 °C（注 2）

| 驱动区域 | 符号 (Symbol) | 参数说明 (Parameter) | 最小值 | 典型值 | 最大值 | 单位 |
| :--- | :--- | :--- | :---: | :---: | :---: | :---: |
| **高端上桥臂**<br>(High-side) | t<sub>ON</sub> | 开通时间 (Turn-on time) | - | 760 | - | ns |
| | t<sub>c(ON)</sub> | 开通交叉时间 (Turn-on across time) | - | 220 | - | ns |
| | t<sub>rr</sub> | 反向恢复时间 (Reverse recovery time) | - | 130 | - | ns |
| | t<sub>OFF</sub> | 关断时间 (Turn-off time) | - | 570 | - | ns |
| | t<sub>c(OFF)</sub> | 关断交叉时间 (Turn-off across time) | - | 75 | - | ns |
| **低端下桥臂**<br>(Low-side) | t<sub>ON</sub> | 开通时间 (Turn-on time) | - | 900 | - | ns |
| | t<sub>c(ON)</sub> | 开通交叉时间 (Turn-on across time) | - | 300 | - | ns |
| | t<sub>rr</sub> | 反向恢复时间 (Reverse recovery time) | - | 170 | - | ns |
...
| V<sub>CIN(ref)</sub> | 短路保护触发电压阈值 | T<sub>C</sub> = -40 ~ 100 °C, V<sub>D</sub> = 15 V（下桥 SCP 触发点，注 3）| 0.455 | 0.480 | 0.505 | V |
...
| t<sub>DEAD</sub> | 桥臂互锁防直通死区 | 作用于控制 MCU 侧输出的同桥臂上下两路 PWM 信号互锁死区时间限制 | 1.5 | - | - | μs |
