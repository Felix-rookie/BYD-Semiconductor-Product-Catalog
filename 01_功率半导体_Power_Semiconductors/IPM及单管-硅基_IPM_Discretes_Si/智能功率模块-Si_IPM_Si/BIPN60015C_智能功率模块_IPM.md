...
| ± I<sub>C</sub> | 各 IGBT 集电极电流 (Each IGBT collector current) | T<sub>C</sub> = 25 °C | 15 | A |
| I<sub>CP</sub> | 晶圆集电极脉冲电流 (Each IGBT collector current (peak)) | T<sub>C</sub> = 25 °C, 脉宽 < 1 ms | 30 | A |
| P<sub>C</sub> | 集电极功耗 (Collector dissipation) | T<sub>C</sub> = 25 °C, 每一片小芯片 | 50 | W |
...
| :--- | :--- | :--- | :---: | :---: |
| R<sub>th(j-c)Q</sub> | 结点-壳体热阻 (Junction to case) | 逆变器 IGBT 部分 (每 1/6 模块) | 2.02 | °C/W |
| R<sub>th(j-c) F</sub> | 结点-壳体热阻 (Junction to case) | 逆变器 FRD 续流二极管部分 (每 1/6 模块) | 3.15 | °C/W |
...
| :--- | :--- | :--- | :---: | :---: | :---: | :---: |
| V<sub>CE(SAT)</sub> | 集电极-发射极饱和电压 | V<sub>D</sub> = V<sub>BS</sub> = 15 V, V<sub>IN</sub> = 5 V, I<sub>C</sub> = 15 A, T<sub>J</sub> = 25 °C | - | 1.9 | 2.3 | V |
| V<sub>F</sub> | FRD 正向导通电压 | V<sub>IN</sub> = 0 V, I<sub>C</sub> = 10 A, T<sub>J</sub> = 25 °C | - | 1.6 | 2.0 | V |
...
测试条件：V<sub>PN</sub> = 300 V, V<sub>D</sub> = V<sub>BS</sub> = 15 V, I<sub>C</sub> = 15 A, V<sub>IN</sub> = 0 ↔ 5 V, T<sub>J</sub> = 25 °C（注 2）

| 驱动区域 | 符号 (Symbol) | 参数说明 (Parameter) | 最小值 | 典型值 | 最大值 | 单位 |
| :--- | :--- | :--- | :---: | :---: | :---: | :---: |
| **高端上桥臂**<br>(High-side) | t<sub>ON</sub> | 开通时间 (Turn-on time) | - | 680 | - | ns |
| | t<sub>c(ON)</sub> | 开通交叉时间 (Turn-on across time) | - | 200 | - | ns |
| | t<sub>OFF</sub> | 关断时间 (Turn-off time) | - | 850 | - | ns |
| | t<sub>c(OFF)</sub> | 关断交叉时间 (Turn-off across time) | - | 150 | - | ns |
| **低端下桥臂**<br>(Low-side) | t<sub>ON</sub> | 开通时间 (Turn-on time) | - | 630 | - | ns |
| | t<sub>c(ON)</sub> | 开通交叉时间 (Turn-on across time) | - | 240 | - | ns |
| | t<sub>OFF</sub> | 关断时间 (Turn-off time) | - | 850 | - | ns |
| | t<sub>c(OFF)</sub> | 关断交叉时间 (Turn-off across time) | - | 160 | - | ns |
...
| :--- | :--- | :--- | :---: | :---: | :---: | :---: |
| I<sub>DL</sub> | 低端低压供电静态电流 | V<sub>D</sub> = 15 V, IN<sub>(LU, LV, LW)</sub> = 0 V，加在 V<sub>D(L)</sub> - COM | - | - | 600 | μA |
...
| V<sub>FOH</sub> | 故障输出高电平电压 | V<sub>CIN</sub> = 0 V，故障引脚外接 4.7 kΩ 阻值上拉到 5 V 电源 | 4.5 | - | - | V |
| V<sub>FOL</sub> | 故障输出低电平电压 | V<sub>CIN</sub> = 1 V，故障引脚外接 4.7 kΩ 阻值上拉到 5 V 电源 | - | - | 0.8 | V |
...
| UV<sub>BSD</sub> | 高端自举电路欠压检测 | 自举浮动供电监测（电压下降脱扣） | 9.0 | 10.0 | 10.9 | V |
| UV<sub>BSR</sub> | 高端自举电路欠压复位 | 自举浮动供电监测（电压回升恢复） | 10.1 | 11.0 | 11.9 | V |
| t<sub>FO</sub> | 故障输出脉冲宽度 | 采样端滤波配置电容 C<sub>FO</sub> = 26 nF（注 4）<br>采样端滤波配置电容 C<sub>FO</sub> = 33 nF（注 4） | 0.8<br>- | -<br>2.30 | -<br>- | ms |
| V<sub>IN(ON)</sub> | 控制信号导通阈值电压 | 输入高电平有效（逻辑由低到高），施加在输入引脚与 COM 之间 | - | - | 3.0 | V |
| V<sub>IN(OFF)</sub> | 控制信号关断阈值电压 | 输入低电平有效（逻辑由高到低），施加在输入引脚与 COM 之间 | 0.8 | - | - | V |

> **注 3**：短路/过流保护功能仅在模块的**低端下桥臂**驱动电路中有效。
> **注 4**：故障信号输出脉宽 t<sub>FO</sub> 的持续时长由外部配置电容 C<sub>FO</sub> 的容值直接控制，其近似工程设计推导计算公式为：C<sub>FO</sub> ≈ 14.3 × 10<sup>-6</sup> × t<sub>FO</sub> [F]。
