# 比亚迪半导体 (BYD) 家电级 IPM 智能功率模块 - BIP60010B
...
**BIP60010B** 是一款紧凑型全塑封智能功率模块 (IPM)。该产品内部集成高可靠性的高压栅极驱动电路，内置带限流电阻的自举二极管，集成欠压、过流和使能关断等保护功能，自带温度输出功能，提供了优异的保护和较宽的安全工作范围，是小功率白色家电的理想选择。由于采用了分立的负端子，可独立监测逆变器的每一相电流。
...
| :--- | :---: | :---: | :---: | :---: | :---: |
| **BIP60010B** | 600 V | 10 A | 1650 Vrms/min | 三相六管全桥 (Three-Phase Inverter) | DIP25-B-3215 |
...
| ± I<sub>C</sub> | 各 IGBT 集电极持续电流 (Each IGBT collector current) | T<sub>C</sub> = 25 °C, T<sub>J</sub> ≤ 150 °C | 10 | A |
| I<sub>CP</sub> | 晶圆集电极峰值电流 (Each IGBT collector peak current) | T<sub>C</sub> = 25 °C, T<sub>J</sub> ≤ 150 °C, 脉宽 < 1 ms | 20 | A |
| P<sub>C</sub> | 集电极功耗 (Collector dissipation) | T<sub>C</sub> = 25 °C, 每一片小芯片 | 24 | W |
...
| :--- | :--- | :--- | :---: | :---: |
| R<sub>th(j-c)Q</sub> | 结点-壳体热阻 (Junction to case) | 逆变器 IGBT 部分 (每 1/6 模块) | 5.2 | °C/W |
| R<sub>th(j-c)F</sub> | 结点-壳体热阻 (Junction to case) | 逆变器 FRD 续流二极管部分 (每 1/6 模块) | 6.9 | °C/W |
...
| :--- | :--- | :--- | :---: | :---: | :---: | :---: |
| V<sub>CE(SAT)</sub> | 集电极-发射极饱和电压 | V<sub>D</sub> = V<sub>BS</sub> = 15 V, V<sub>IN</sub> = 5 V, I<sub>C</sub> = 10 A, T<sub>J</sub> = 25 °C | - | 1.7 | 2.2 | V |
| V<sub>F</sub> | FRD 正向导通电压 | V<sub>IN</sub> = 0 V, I<sub>F</sub> = 10 A, T<sub>J</sub> = 25 °C | - | 1.8 | 2.3 | V |
...
测试条件：V<sub>PN</sub> = 300 V, V<sub>D</sub> = V<sub>BS</sub> = 15 V, I<sub>C</sub> = 10 A, V<sub>IN</sub> = 0 ↔ 5 V, T<sub>J</sub> = 25 °C（注 2）
