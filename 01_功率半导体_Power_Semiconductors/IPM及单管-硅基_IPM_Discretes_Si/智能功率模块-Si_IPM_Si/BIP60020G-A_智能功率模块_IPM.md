# 比亚迪半导体 (BYD) 家电级 IPM 智能功率模块 - BIP60020G-A
...
**BIP60020G-A** 是一款集成度极高的超小型封装三相逆变智能功率模块 (IPM)。该模块采用先进设计，在极度紧凑的封装内实现了优化匹配的门极驱动与低损耗 IGBT 功率器件，并内置了全面的电路保护功能。由于采用了陶瓷覆铜板 (DBC) 和分立的负直流端子设计，产品在超小体积下实现了优异的低热阻性能与独立的相电流监测，同时内置温度模拟电压输出功能。

### 🌟 核心特性 (Features)
* **高度集成**：600V / 20A 三相 IGBT 逆变全桥，内置自举二极管与高压集成电路 (HVIC)，实现单电源、无需光耦隔离的门极驱动能力。
...
| :--- | :---: | :---: | :---: | :---: | :---: |
| **BIP60020G-A** | 600 V | 20 A | 1500 Vrms/min | 三相六管全桥 (Three-Phase Inverter) | BIP25-3824 |
...
| ± I<sub>C</sub> | 各 IGBT 集电极电流 (Each IGBT collector current) | T<sub>C</sub> = 25 °C | 20 | A |
| I<sub>CP</sub> | 晶圆集电极脉冲电流 (Each IGBT collector current (peak)) | T<sub>C</sub> = 25 °C, 脉宽 < 1 ms | 40 | A |
| P<sub>C</sub> | 集电极功耗 (Collector dissipation) | T<sub>C</sub> = 25 °C, 每一片小芯片 | 69 | W |
...
| I<sub>FO</sub> | 故障输出电流 (Fault output current) | 在 V<sub>FO</sub> 端子的最大允许灌电流 | 2 | mA |
...
| V<sub>ISO</sub> | 绝缘内部耐压 (Isolation voltage) | 60 Hz 正弦波，交流电压持续 1 分钟，所有连接引脚到 DBC 之间 | 1500 | Vrms |
...
| :--- | :--- | :--- | :---: | :---: |
| R<sub>th(j-c)Q</sub> | 结点-壳体热阻 (Junction to case) | 逆变器 IGBT 部分 (每 1/6 模块) | 1.8 | °C/W |
| R<sub>th(j-c)F</sub> | 结点-壳体热阻 (Junction to case) | 逆变器 FRD 续流二极管部分 (每 1/6 模块) | 2.3 | °C/W |
...
| :--- | :--- | :--- | :---: | :---: | :---: | :---: |
| V<sub>CE(SAT)</sub> | 集电极-发射极饱和电压 | V<sub>D</sub> = V<sub>BS</sub> = 15 V, V<sub>IN</sub> = 5 V, I<sub>C</sub> = 20 A, T<sub>J</sub> = 25 °C | - | 1.7 | 2.2 | V |
| V<sub>F</sub> | FRD 正向导通电压 | V<sub>IN</sub> = 0 V, I<sub>C</sub> = 20 A, T<sub>J</sub> = 25 °C | - | 1.8 | 2.3 | V |
...
测试条件：V<sub>PN</sub> = 400 V, V<sub>D</sub> = V<sub>BS</sub> = 15 V, I<sub>C</sub> = 20 A, V<sub>IN</sub> = 0 ↔ 5 V, T<sub>J</sub> = 25 °C（注 2）

| 驱动区域 | 符号 (Symbol) | 参数说明 (Parameter) | 最小值 | 典型值 | 最大值 | 单位 |
| :--- | :--- | :--- | :---: | :---: | :---: | :---: |
| **高端上桥臂**<br>(High-side) | t<sub>ON</sub> | 开通时间 (Turn-on time) | - | 700 | - | ns |
| | t<sub>c(ON)</sub> | 开通交叉时间 (Turn-on across time) | - | 160 | - | ns |
| | t<sub>rr</sub> | 反向恢复时间 (Reverse recovery time) | - | 120 | - | ns |
| | t<sub>OFF</sub> | 关断时间 (Turn-off time) | - | 620 | - | ns |
| | t<sub>c(OFF)</sub> | 关断交叉时间 (Turn-off across time) | - | 100 | - | ns |
| **低端下桥臂**<br>(Low-side) | t<sub>ON</sub> | 开通时间 (Turn-on time) | - | 850 | - | ns |
| | t<sub>c(ON)</sub> | 开通交叉时间 (Turn-on across time) | - | 250 | - | ns |
| | t<sub>rr</sub> | 反向恢复时间 (Reverse recovery time) | - | 145 | - | ns |
| | t<sub>OFF</sub> | 关断时间 (Turn-off time) | - | 650 | - | ns |
| | t<sub>c(OFF)</sub> | 关断交叉时间 (Turn-off across time) | - | 80 | - | ns |
...
| :--- | :--- | :--- | :---: | :---: | :---: | :---: |
| I<sub>DL</sub> | 低端 VD 静态电流 | V<sub>D(L)</sub> = 15 V, IN<sub>(LU, LV, LW)</sub> = 0 V，V<sub>D(L)</sub> - COM(L) | - | - | 1.0 | mA |
| I<sub>DH</sub> | 高端 VD 静态电流 | V<sub>D(H)</sub> = 15 V, IN<sub>(HU, HV, HW)</sub> = 0 V，V<sub>D(HW)</sub> - COM(H) | - | - | 600 | μA |
| I<sub>QBS</sub> | VBS 静态电流 | V<sub>BS</sub> = 15 V, IN<sub>(HU, HV, HW)</sub> = 0 V，自举回路 | - | - | 200 | μA |
...
| V<sub>CIN(ref)</sub> | 短路保护触发电压阈值 | T<sub>C</sub> = -40 ~ 100 °C, V<sub>D</sub> = 15 V（下桥 SCP 触发点，注 3）| 0.415 | 0.46 | 0.505 | V |
...
| t<sub>FO</sub> | 故障输出脉冲宽度 | 供电电路欠压保护或短路保护 | 40 | - | - | μs |
...
| f<sub>PWM</sub> | PWM 调制载波频率 | 系统推荐开关载波工作范围，-40 °C ≤ T<sub>J</sub> ≤ 125 °C, T<sub>C</sub> ≤ 100 °C| - | - | 20 | kHz |
